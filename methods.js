...
const getAllCabinetBoxes = async () => {
    ...
    return cabinetBoxes.map(box => box.toJSON());
}

const getMedicationDetailsBasedOnBox = async () => {
    const cabinetBoxes = await getAllCabinetBoxes();
    var medicationDetails = [];
    for (var box of cabinetBoxes) {
        const medication = await getMedicationById(box.medication_id);
...
const patient = async (req, res) => {
    if (req.session.user) {
        try {
            ...
            const medicationDetails = await getMedicationDetailsBasedOnBox(); //getMedicationsWithDetails(patientDetails.id);
